...
pub fn build(b: *Builder) void {
    ...
    const exe = b.addExecutable(.{ .name = "pars", .root_source_file = .{ .path = "src/main.zig" }, .optimize = .ReleaseSafe, .version = .{ .major = version.major, .minor = version.minor, .patch = version.patch } });
